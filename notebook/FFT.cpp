...
// RESULT: $out[k]=\sum_{j=0}^{size-1}in[j] e^{(dir)2\pi{}ijk/size}$
...
// Usage:
// $f[0\cdots{}N-1]$ and $g[0\cdots{}N-1]$ are numbers
// Want to compute the convolution $h$, defined by
// $h[n]=\sum_{k=0}^{N-1}f[k] g[n-k]$
// Here, the index is cyclic; $f[-1]=f[N-1]$, $f[-2]=f[N-2]$, etc.
// Let $F[0\cdots{}N-1]$ be FFT($f$), and similarly, define $G$ and $H$.
